...
) : IUpdateSource
{
    ...
    )
    {
        ...
        try
        {
            var version = await mirrorChyanService.GetLatestVersionAsync(
                currentSourceOptions.Cdk,
                currentSourceOptions.Channel
            );

            if (version.Artifact is null)
            {
                throw new ResourceNotDownloadableException(
                    "Resource is not available. Maybe cdk is not provided."
                );
            }
            ...
            var asset = new VelopackAsset
            {
                ...
                Version = SemanticVersion.Parse(
                    version.Version.StartsWith("v", StringComparison.OrdinalIgnoreCase)
                        ? version.Version[1..]
                        : version.Version
                ),
                Type =
                    version.Artifact.Kind == UpdateKind.Full
                        ? VelopackAssetType.Full
                        : VelopackAssetType.Delta,
                FileName = fileName,
                SHA256 = version.Artifact.Sha256.ToUpper(),
    ...
    )
    {
        ...
        if (
            !_downloadUrls.TryGetValue(releaseEntry.FileName, out var version)
            || version.Artifact is null
        )
        {
        ...
        logger.Info(
            $"Downloading {releaseEntry.FileName} from MirrorChyan: {version.Artifact.Url}"
        );
